    ...
    extends RuntimeClientLaunchDelegate
{
        ...
        final InfobaseReference infobase, final IProgressMonitor monitor) throws CoreException
    {
        ...
        if (v8project instanceof IExternalObjectProject)
        {
            ...
            if (externalObjectDumpSupport.isEnabled(externalObjectProject.getProject()))
            {
                ...
                {
                    arguments.setExternalObjectDumpPath(externalObjectDumpSupport.getDump(
                        externalObjectProject.getProject(), externalObject, true, monitor));
                }
                else
                {
        ...
        final String externalObjectStartupOptions = configuration.getAttribute(
            UnitTestLaunchConfigurationAttributes.EXTERNAL_OBJECT_STARTUP_OPTIONS, (String)null);
    ...
    private String getFeaturesPath(ILaunchConfiguration configuration)
    {
        ...
        {
            LaunchingPlugin.log(LaunchingPlugin.createErrorStatus(
                Messages.RuntimeUnitLauncherLaunchDelegate_Incorrect_launch_configuration, e));
        ...
        throws CoreException
    {
        ...
        try
        {
            final RuntimeInstallation installation = resolvable.get(
                candidate -> runtimeComponentManager.hasComponent(candidate, new String[] { clientTypeId })
                    && matchesByArch(candidate, infobase),
                new ArchPriorityComparator(infobase));
        ...
        final RuntimeExecutionArguments arguments, final Publication publication) throws CoreException
    {
        ...
        final boolean canRunRuntimeClient =
            runtimeComponentManager.getTypes(IRuntimeClientLauncher.class).stream().anyMatch(
                type -> clientTypeId.equals(type.getId()));
        try
        {
    ...
    protected IStatus isValid(ILaunchConfiguration configuration, String mode) throws CoreException
    {
        ...
        String externalObjectDumpPath =
            configuration.getAttribute(UnitTestLaunchConfigurationAttributes.EXTERNAL_OBJECT_DUMP_PATH, (String)null);
        String externalObjectStartupOptions = configuration.getAttribute(
            UnitTestLaunchConfigurationAttributes.EXTERNAL_OBJECT_STARTUP_OPTIONS, (String)null);
