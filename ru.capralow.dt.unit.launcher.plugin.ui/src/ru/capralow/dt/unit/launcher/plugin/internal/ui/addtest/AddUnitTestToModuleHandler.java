...
public class AddUnitTestToModuleHandler extends AbstractHandler {
	...
	public void execute(XtextEditor target) {
		...
		if (configuration == null) {
			String msg = MessageFormat.format(Messages.AddUnitTest_Unable_to_get_configuration_from_base_project_0,
					v8Project);
		...
		String testMethodContent = readContents(
				getFileInputSupplier("/resources/unitTestMethod" + lang + ".txt", UnitLauncherUiPlugin.ID)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		try {
			...
			UnitLauncherUiPlugin.log(UnitLauncherUiPlugin.createErrorStatus(e.getMessage(), e));
