// @unit-test
Процедура ПроверитьРавенствоСтрокиНаИстину(Фреймворк) Экспорт
	ЭталоннаяСтрока		= "Эталон";
	ПроверяемаяСтрока	= "Эталон";
	
	Фреймворк.ПроверитьРавенство(ЭталоннаяСтрока, ПроверяемаяСтрока, "Строки равны.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьРавенствоСтрокиНаЛожь(Фреймворк) Экспорт
	ЭталоннаяСтрока		= "Эталон";
	ПроверяемаяСтрока	= "Результат";
	
	Фреймворк.ПроверитьРавенство(ЭталоннаяСтрока, ПроверяемаяСтрока, "Строки ошибочно неравны.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьНеРавенствоСтрокиНаИстину(Фреймворк) Экспорт
	ЭталоннаяСтрока		= "Эталон";
	ПроверяемаяСтрока	= "Результат";
	
	Фреймворк.ПроверитьНеРавенство(ЭталоннаяСтрока, ПроверяемаяСтрока, "Строки неравны.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьНеРавенствоСтрокиНаЛожь(Фреймворк) Экспорт
	ЭталоннаяСтрока		= "Эталон";
	ПроверяемаяСтрока	= "Эталон";
	
	Фреймворк.ПроверитьНеРавенство(ЭталоннаяСтрока, ПроверяемаяСтрока, "Строки ошибочно равны.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьВхождениеСтрокиНаИстина(Фреймворк) Экспорт
	ПолнаяСтрока	= "ФреймворкТестирования";
	ПодстрокаПоиска	= "Тест";
	
	Фреймворк.ПроверитьВхождение(ПолнаяСтрока, ПодстрокаПоиска, "Подстрока входит в строку.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьВхождениеСтрокиНаЛожь(Фреймворк) Экспорт
	ПолнаяСтрока	= "ФреймворкТестирования";
	ПодстрокаПоиска	= "Релиз";
	
	Фреймворк.ПроверитьВхождение(ПолнаяСтрока, ПодстрокаПоиска, "Подстрока ошибочно невходит в строку.");
КонецПроцедуры
