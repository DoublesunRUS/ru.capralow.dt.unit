// @unit-test
Процедура ПроверитьВыполнилосьНаИстину(Фреймворк) Экспорт
	Алгоритм	= "ПроверитьИстину(Истина, ""Результат Истина."")";
	
	Фреймворк.ПроверитьВыполнилось(Алгоритм, Неопределено, "Строка встроенного языка выполнилась.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьВыполнилосьНаЛожь(Фреймворк) Экспорт
	Алгоритм	= "ПроверитьИстину(Ложь, ""Результат Не Истина."")";
	
	// FIXME: В Expected/Actual должно быть Истина/Ложь
	Фреймворк.ПроверитьВыполнилось(Алгоритм, Неопределено, "Строка встроенного языка ошибочно невыполнилась.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьНеВыполнилосьНаИстину(Фреймворк) Экспорт
	Алгоритм	= "ПроверитьЛожь(Истина, ""Результат Ложь."")";
	
	// FIXME: Проверка упала, а не должна
	Фреймворк.ПроверитьНеВыполнилось(Алгоритм, Неопределено, "Строка встроенного языка выполнилась.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьНеВыполнилосьНаЛожь(Фреймворк) Экспорт
	Алгоритм	= "ПроверитьЛожь(Ложь, ""Результат Не Ложь."")";
	
	// FIXME: В Expected/Actual должно быть Ложь/Истина
	Фреймворк.ПроверитьНеВыполнилось(Алгоритм, Неопределено, "Строка встроенного языка ошибочно выполнилась.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьМетодВыполнилсяНаИстина(Фреймворк) Экспорт
	ИмяМетода	= "ПроверитьИстину";
	Параметры	= Новый Массив;
	Параметры.Добавить("Истина");
	
	Фреймворк.ПроверитьМетодВыполнился(Фреймворк, ИмяМетода, Параметры, "Метод выполнился.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьМетодВыполнилсяНаЛожь(Фреймворк) Экспорт
	ИмяМетода	= "ПроверитьИстину";
	Параметры	= Новый Массив;
	Параметры.Добавить("Ложь");
	
	// FIXME: В Expected/Actual должно быть Ложь/Истина
	Фреймворк.ПроверитьМетодВыполнился(Фреймворк, ИмяМетода, Параметры, "Метод ошибочно невыполнился.");
КонецПроцедуры

// @unit-test
Процедура ПроверитьМетодНеВыполнилсяНаИстина(Фреймворк) Экспорт
	ИмяМетода	= "ПроверитьЛожь";
	Параметры	= Новый Массив;
	Параметры.Добавить("Истина");
	
	// FIXME: В Expected/Actual должны быть сообщения об ошибках
	ТекстОшибки = 
	"{ВнешняяОбработка.VanessaAutomationsingle.Форма.УправляемаяФорма.Форма(18042)}: [Failed] Переданный параметр (Истина) не является Ложью, а хотели, чтобы являлся.
	|ВызватьИсключение ТекстСообщения;";
	Фреймворк.ПроверитьМетодНеВыполнился(Фреймворк, ИмяМетода, Параметры, ТекстОшибки);
КонецПроцедуры

// @unit-test
Процедура ПроверитьМетодНеВыполнилсяНаЛожь(Фреймворк) Экспорт
	ИмяМетода	= "ПроверитьЛожь";
	Параметры	= Новый Массив;
	Параметры.Добавить("Ложь");
	
	// FIXME: В Expected/Actual должно быть Ложь/Истина
	Фреймворк.ПроверитьМетодНеВыполнился(Фреймворк, ИмяМетода, Параметры, "");
КонецПроцедуры

